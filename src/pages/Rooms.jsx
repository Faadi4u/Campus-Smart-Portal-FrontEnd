...
const Rooms = () => {
  ...
  }, [loading, activeFilter, filteredRooms]); // Re-run animation when filter changes
  ...
  const openModal = () => {
    ...
    setTimeout(() => {
      ...
      gsap.fromTo(modalContentRef.current, { scale: 0.9, opacity: 0, y: 20 }, { scale: 1, opacity: 1, y: 0, duration: 0.4, ease: "back.out(1.7)" });
    }, 10);
  };

  const closeModal = () => {
    gsap.to(modalRef.current, { opacity: 0, duration: 0.3 });
    gsap.to(modalContentRef.current, { scale: 0.9, opacity: 0, y: 20, duration: 0.3, onComplete: () => setShowModal(false) });
  };

  if (loading) return <div className="flex justify-center items-center h-full min-h-[50vh]"><Loader2 className="w-8 h-8 animate-spin text-indigo-500"/></div>;
    ...
    <div ref={containerRef} className="space-y-6">
      ...
      {filteredRooms.length === 0 ? (
        <div className="text-center py-20 bg-zinc-900/30 rounded-2xl border border-zinc-800/50">
          ...
          <h3 className="text-xl font-medium text-white">No {activeFilter === "all" ? "rooms" : activeFilter}s found</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredRooms.map((room) => (
            <div 
              ...
              className={`room-card group bg-zinc-900/40 backdrop-blur-sm p-6 rounded-2xl border border-white/5 hover:border-indigo-500/30 hover:bg-zinc-800/40 transition-all duration-300 hover:shadow-xl hover:shadow-indigo-500/10 hover:-translate-y-1 ${user?.role !== 'admin' ? 'cursor-pointer' : ''}`}
            >
      ...
      {/* Add Room Modal (Same as before) */}
      {showModal && (
        <div ref={modalRef} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 opacity-0">
          <div ref={modalContentRef} className="bg-zinc-900 border border-zinc-700 rounded-2xl max-w-md w-full p-0 shadow-2xl overflow-hidden opacity-0 scale-90">
            <div className="p-6 border-b border-zinc-800 flex justify-between items-center">
              <h2 className="text-xl font-bold text-white">Add New Room</h2>
              <button onClick={closeModal} className="text-zinc-400 hover:text-white hover:bg-zinc-800 p-1 rounded-lg transition-colors"><X size={20} /></button>
            </div>
            ...
            <form onSubmit={handleCreate} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input type="text" placeholder="e.g. Lab 1" required className="w-full bg-zinc-800/50 border border-zinc-700 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})}/>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-1.5">Type</label>
                  <select className="w-full bg-zinc-800/50 border border-zinc-700 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all appearance-none" value={formData.type} onChange={e => setFormData({...formData, type: e.target.value})}>
                ...
                <div>
                  ...
                  <input type="number" placeholder="e.g. 50" required className="w-full bg-zinc-800/50 border border-zinc-700 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all" value={formData.capacity} onChange={e => setFormData({...formData, capacity: e.target.value})}/>
                </div>
              </div>
              {/* Location & Features */}
              <div><label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-1.5">Location</label><input type="text" placeholder="e.g. Block A" required className="w-full bg-zinc-800/50 border border-zinc-700 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all" value={formData.location} onChange={e => setFormData({...formData, location: e.target.value})}/></div>
              <div><label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-1.5">Features</label><input type="text" placeholder="e.g. Projector, AC" className="w-full bg-zinc-800/50 border border-zinc-700 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all" value={formData.features} onChange={e => setFormData({...formData, features: e.target.value})}/></div>
              
              <div className="flex justify-end gap-3 mt-8 pt-4 border-t border-zinc-800">
                <button type="button" onClick={closeModal} className="px-4 py-2.5 text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-xl transition-colors font-medium text-sm">Cancel</button>
                <button type="submit" disabled={creating} className="px-6 py-2.5 bg-indigo-600 text-white rounded-xl hover:bg-indigo-500 shadow-lg shadow-indigo-900/20 font-medium text-sm transition-all flex items-center gap-2">{creating ? <Loader2 size={16} className="animate-spin" /> : null} {creating ? "Creating..." : "Create Room"}</button>
              </div>
            </form>
          </div>
        </div>
